...
src = data['english']
exp_reply = data['korean']
src = src.values.tolist()
exp_reply = exp_reply.values.tolist()
...
for question in src:
    if count == 100:
        break
    reply = prediction(question)
    model_replies.append(reply)
    count += 1

df = pd.DataFrame(list(zip(src, model_replies, exp_reply)), columns=['Input', 'Reply', 'Expected Reply'])
